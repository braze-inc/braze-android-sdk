...
object EventSubscriberFactory {
    ...
    fun createInAppMessageEventSubscriber(config: UnityConfigurationProvider): IEventSubscriber<InAppMessageEvent> =
        IEventSubscriber { inAppMessageEvent: InAppMessageEvent ->
    ...
    fun createContentCardsEventSubscriber(config: UnityConfigurationProvider): IEventSubscriber<ContentCardsUpdatedEvent> =
        IEventSubscriber { contentCardsUpdatedEvent: ContentCardsUpdatedEvent ->
    ...
    fun createFeatureFlagsEventSubscriber(config: UnityConfigurationProvider): IEventSubscriber<FeatureFlagsUpdatedEvent> =
        IEventSubscriber { featureFlagsUpdatedEvent: FeatureFlagsUpdatedEvent ->
    ...
    fun createSdkAuthenticationFailureSubscriber(config: UnityConfigurationProvider): IEventSubscriber<BrazeSdkAuthenticationErrorEvent> =
        IEventSubscriber { sdkAuthErrorEvent: BrazeSdkAuthenticationErrorEvent ->
    ...
    fun createPushEventSubscriber(config: UnityConfigurationProvider): IEventSubscriber<BrazePushEvent> =
        IEventSubscriber { event: BrazePushEvent ->
            val (callback, gameObject) =
                when (event.eventType) {
                    BrazePushEventType.NOTIFICATION_RECEIVED ->
                        Pair(
                            config.pushReceivedCallbackMethodName,
                            config.pushReceivedGameObjectName,
                        )
                    BrazePushEventType.NOTIFICATION_DELETED ->
                        Pair(
                            config.pushDeletedCallbackMethodName,
                            config.pushDeletedGameObjectName,
                        )
                    BrazePushEventType.NOTIFICATION_OPENED ->
                        Pair(
                            config.pushOpenedCallbackMethodName,
                            config.pushOpenedGameObjectName,
                        )
                }
